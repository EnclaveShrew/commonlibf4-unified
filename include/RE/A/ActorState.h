...
class __declspec(novtable) ActorState : public IMovementState // 00
{
  public:
    ...
    [[nodiscard]] bool GetWeaponMagicDrawn() const noexcept
    {
        return weaponState >= WEAPON_STATE::kDrawn;
    }
    [[nodiscard]] bool IsSwimming() const noexcept
    {
        return DoGetMoveModeBits(0x400);
    }
  ...
  private:
    bool DoGetMoveModeBits(std::uint16_t a_bits) const
    {
        return (a_bits & moveMode & 0x3FFF) == a_bits;
    }
};
