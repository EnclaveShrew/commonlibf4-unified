...
template <class> class BSPointerHandleSmartPointer;
...
class __declspec(novtable) Actor : public TESObjectREFR,                                          // 000
                                   ...
                                   public BSTEventSource<ActorCPMEvent>                           // 278
{
  public:
    ...
    virtual void GoToPrison([[maybe_unused]] TESFaction *a_faction, [[maybe_unused]] bool a_removeItems,
                            [[maybe_unused]] bool a_realJail)
    {
        return;
    } // 0DA
    ...
    virtual void PayFine([[maybe_unused]] TESFaction *a_faction, [[maybe_unused]] bool a_movetoMarker,
                         [[maybe_unused]] bool a_removeStolenItems)
    {
        return;
    } // 0DC
    ...
    virtual ObjectRefHandle DropObject(const BGSObjectInstance &a_object, BSTSmallArray<std::uint32_t, 4> *a_stackData,
                                       std::int32_t a_number, const NiPoint3 *a_point, const NiPoint3 *a_rotate); // 0EB
    ...
    virtual std::uint32_t UseAmmo(const BGSObjectInstanceT<TESObjectWEAP> &a_weapon, BGSEquipIndex a_equipIndex,
                                  std::uint32_t a_shotCount);                                                     // 0F0
    ...
    virtual void InitiateFlee(TESObjectREFR *a_fleeRef, bool a_runonce, bool a_knows, bool a_combatMode,
                              TESObjectCELL *a_cell, TESObjectREFR *a_ref, float a_fleeFromDist,
                              float a_fleeToDist); // 0F8
    virtual void InitiateGetUpPackage();           // 0F9
    virtual void PutCreatedPackage(TESPackage *a_pack, bool a_tempPack, bool a_isACreatedPackage,
                                   bool a_allowFromFurniture); // 0FA
    ...
    virtual bool IsAtPoint(const NiPoint3 &a_point, float a_radius, bool a_expandRadius,
                           bool a_alwaysTestHeight);                                             // 109
    ...
    virtual bool ShouldPivotToFaceCamera() const
    {
        return false;
    } // 115
    ...
    virtual void SetLastRiddenMount([[maybe_unused]] ActorHandle a_mount)
    {
        return;
    } // 11C
    ...
    virtual bool ComputeMotionFeedbackSpeedAndDirection(const ActorMotionFeedbackData &a_data, float a_delta,
                                                        ActorMotionFeedbackOutput &a_retData);  // 128
    ...
    virtual bool ShouldRespondToActorCollision(
        const MovementMessageActorCollision &,
        const BSPointerHandleSmartPointer<BSPointerHandleManagerInterface<Actor, HandleManager>> &a_otherActor)
        const;                                                                     // 130
    virtual float CheckClampDamageModifier(ActorValueInfo &a_info, float a_delta); // 131
    virtual void ValidateNewPath([[maybe_unused]] const MovementMessageNewPath &a_newPathMessage)
    {
        return;
    } // 132
    ...
    bool HasObjects(TESBoundObject *a_obj, PACKAGE_OBJECT_TYPE a_formID, std::int32_t a_number, std::uint32_t a_id,
                    PACKAGE_OBJECT_TYPE &a_matchID)
    {
    ...
    [[nodiscard]] bool IsEquipReady() const
    {
        using func_t = decltype(&Actor::IsEquipReady);
        static REL::Relocation<func_t> func{ID::Actor::IsEquipReady};
        return func(this);
    }
    ...
    BSSoundHandle &PlaySoundByEditorName(BSSoundHandle &a_handle, const char *a_editorName, bool a_loop,
                                         std::uint32_t a_usageFlags) const
    {
    ...
    void PollItemEquip(bool a_force)
    {
        using func_t = decltype(&Actor::PollItemEquip);
        static REL::Relocation<func_t> func{ID::Actor::PollItemEquip};
