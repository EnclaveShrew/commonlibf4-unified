...
{
template <class T>
concept ExtraDataListConstraint = std::derived_from<T, BSExtraData> && !std::is_pointer_v<T> && !std::is_reference_v<T>;
}

class ExtraDataList : public BSIntrusiveRefCounted // 00
{
  public:
    ...
    template <detail::ExtraDataListConstraint T> [[nodiscard]] T *GetByType() const noexcept
    {
    ...
    template <detail::ExtraDataListConstraint T> [[nodiscard]] bool HasType() const noexcept
    {
    ...
    template <detail::ExtraDataListConstraint T> inline std::unique_ptr<T> RemoveExtra()
    {
        return std::unique_ptr<T>{static_cast<T *>(RemoveExtra(T::TYPE).release())};
    }

    inline bool SetBendableSplineInfo(float *a_thickness, float *a_slack, NiPoint3 *a_halfExtents = nullptr,
                                      bool *a_detachedEnd = nullptr)
    {
    ...
    inline void SetDisplayNameFromInstanceData(BGSObjectInstanceExtra *a_instExtra, TESBoundObject *a_object,
                                               const BSTSmartPointer<TBO_InstanceData> &a_data)
    {
