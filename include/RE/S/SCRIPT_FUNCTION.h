...
{
  public:
    using ConditionFunction_t = bool(ConditionCheckParams &a_data, void *a_param2, void *a_param1,
                                     float &a_returnValue);
    using CompileFunction_t = bool(const std::uint16_t a_paramCount, const SCRIPT_PARAMETER *a_parameters,
                                   SCRIPT_LINE *a_scriptLine, ScriptCompileData *a_compileData);
    using ExecuteFunction_t = bool(const SCRIPT_PARAMETER *a_parameters, const char *a_compiledParams,
                                   TESObjectREFR *a_refObject, TESObjectREFR *a_container, Script *a_script,
                                   ScriptLocals *a_scriptLocals, float &a_returnValue, std::uint32_t &a_offset);
    ...
    [[nodiscard]] static auto GetConsoleFunctions()
    {
        ...
        const std::size_t size = REX::FModule::IsRuntimeOG()              ? 521
                                 : REX::FModule::IsRuntimeNG()            ? 525
                                 : version >= REL::Version{1, 11, 221, 0} ? 524
                                                                          : 523;
    ...
    template <std::uint16_t SIZE> inline void SetParameters(SCRIPT_PARAMETER (&a_params)[SIZE])
    {
        paramCount = SIZE;
        parameters = a_params;
    }
