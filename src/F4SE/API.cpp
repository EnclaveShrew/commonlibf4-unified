...
namespace Impl
{
struct API : public REX::TSingleton<API>
{
...
void API::Init(InitInfo a_info, const F4SE::QueryInterface *a_intfc)
{
    ...
    std::call_once(once, [&]() {
        ...
        else
        {
            ...
            const auto size =
                REX::W32::GetModuleFileNameA(REX::W32::GetCurrentModule(), buf.data(), REX::W32::MAX_PATH);
            if (size)
            {
    ...
    if (info.log)
    {
        ...
        std::call_once(once, [&]() {
            ...
            const auto knownResult = REX::W32::SHGetKnownFolderPath(
                REX::W32::FOLDERID_Documents, REX::W32::KF_FLAG_DEFAULT, nullptr, std::addressof(knownBuffer));
            std::unique_ptr<wchar_t[], decltype(&REX::W32::CoTaskMemFree)> knownPath(knownBuffer,
            ...
            path /= std::format("My Games/{}/F4SE/{}.log", GetSaveFolderName(),
                                info.logName ? info.logName : GetPluginName());
            ...
            if (info.logRotate > 0)
            {
                ...
                sinks.push_back(std::make_shared<spdlog::sinks::rotating_file_sink_mt>(path.string(), maxSize,
                                                                                       info.logRotate, true));
            }
            else
            {
